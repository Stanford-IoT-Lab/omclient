...
function Client(config) {
	...
	this._instance = (config.instance) ? config.instance : "";
	...
	if (this._details && this._details.Account) {
		...
		this._amsg = new connection.Connection(undefined, undefined, this._keys, this._privateKey, this._apiKey);
...
Client.prototype.enable = function() {
	...
	if (this.account) {
		...
		if (config.anonymous){
			this._amsg._setReadOnly("/readonlydevice");
			this._amsg.enable();
		} else {
			this._idp.enable();
			this._waitForSignin();
