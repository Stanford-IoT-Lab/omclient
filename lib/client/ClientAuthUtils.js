...
// Returns a URI to a web page for Omlet OAuth
AuthUtils.prototype.getLegacyAuthPage = function(redirectUrl, scopes, cb) {
  if (!scopes) {
    // OmletChat, OmletArcade
    //scopes = ["OmletChat"];
    scopes = ["PublicProfile"];
...
AuthUtils.prototype.confirmLegacyAuth = function(code, queryKey){
...
AuthUtils.prototype.logout = function(cb) {
	var req = new proto.LDDeleteDeviceRequest();
	req.PublicKey = this._client.publicKey;
	this._client.msgCall(req, function(err) {
		if (err) {
			cb(err);
		} else {
