...
function resizeImageCanvas(buf, sizes, cb) {
...
function resizeImageThingPediaPlatform(buf, sizes, cb) {
    var api = platform.getCapability('graphics-api');

    async.parallel(sizes.map(function(size) {
        return function(callback) {
            var image = api.createImageFromBuffer(buf);
            image.resizeFit(size, size);
            image.toBuffer().then(function(buffer) {
                var image2 = api.createImageFromBuffer(buffer);
                return image2.getSize().then(function(actualSize) {
                    return ({ data: buffer,
