...
function LongdanMessageConsumer(client) {
	...
	for (var i = 0; i < CONCURRENT_FETCHES; i++) {
		this._idleFeedFetchWorkers.push(new FeedFetchWorker());
	}
}
...
LongdanMessageConsumer.prototype._enqueueFeedForFetch = function(feedIdentifier) {
	...
	if (fetcher !== undefined) {
		fetcher.fetchNext(this);
	}
}

function FeedFetchWorker() {
}

FeedFetchWorker.prototype.fetchNext = function(lmc) {

	var feedIdentifier = lmc._feedFetchQueue.shift();
	//console.log("pulled next. have " + lmc._feedFetchQueue.length + " feeds queued...");

	if (feedIdentifier === undefined) {
		lmc._idleFeedFetchWorkers.push(this);
		this._consumer = undefined;
		return;
	}

	lmc._feeds.getObjectByKey(feedIdentifier, function(feed) {
		if (!feed) {
			console.warn("Missing feed for sync");
			async.nextTick(this.fetchNext.bind(this, lmc));
		} else {
			...
			this._consumer = lmc;
			this._fetch(lmc, feed);
		}
	}.bind(this));
}

FeedFetchWorker.prototype._fetch = function(lmc, feed) {
